    ...
    func append(jsonArray: [JSON]) {
        ...
        for json in jsonArray {
            for (_ , resource):(String, JSON) in json {
                ...
                let majorData = ResourceDataModel(imageURL: imageURL, resourceCategory: category, subjectName: subject, ownerName: owner, resourceName: srcName, resourceIntro: srcIntro, reviewCounter: "0")
                self.majorDataResults.append(majorData)
            }
        }
    }

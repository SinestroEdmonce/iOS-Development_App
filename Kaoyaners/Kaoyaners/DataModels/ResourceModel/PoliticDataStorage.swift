...
class PoliticDataStorage: NSObject {
    ...
    init(jsonArray: [JSON]) {
        ...
        for json in jsonArray {
            for (_ , resource):(String, JSON) in json {
                ...
                let politicData = ResourceDataModel(imageURL: imageURL, resourceCategory: category, subjectName: subject, ownerName: owner, resourceName: srcName, resourceIntro: srcIntro, reviewCounter: "0")
                self.politicDataResults.append(politicData)
            }
        }
    }

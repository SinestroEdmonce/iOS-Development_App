...
class ImageCollectionViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        // 添加下方工具栏的完成按钮
        self.completeButton = ImageCompleteButton()
        self.completeButton.addTarget(target: self, action: #selector(finishSelect))
        self.completeButton.center = CGPoint(x: UIScreen.main.bounds.width - 50, y: 22)
        self.completeButton.isEnabled = false
        self.toolBar.addSubview(self.completeButton)
    ...
    func collectionView(_ collectionView: UICollectionView,
        ...
        if let cell = collectionView.cellForItem(at: indexPath)
            ...
            if count > self.maxSelected {
                ...
                // 弹出提示
                let title = "最多只能选择\(self.maxSelected)张照片"
